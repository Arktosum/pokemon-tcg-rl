...
def run_fictitious_ppo(args):
    ...
    for episode in range(1, num_episodes + 1):
        ...
        if len(buffer.rewards) > 0:
            advantages, returns = buffer.compute_gae(0.0) # last_value = 0.0
            
            b_states = torch.tensor(np.array(buffer.states), dtype=torch.float32).to(device)
            b_masks = torch.tensor(np.array(buffer.action_masks), dtype=torch.int8).to(device)
            b_actions = torch.tensor(buffer.actions, dtype=torch.int64).to(device)
            b_old_log_probs = torch.tensor(buffer.log_probs, dtype=torch.float32).to(device)
        ...
        if not args.smoke_test and episode % 100 == 0:
            epoch_num = episode // 100
            chk_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "checkpoints", f"TITAN_GREEDY_CURRICULUM_01_epoch_{epoch_num:03d}.pt")
    ...
    if args.smoke_test:
        ...
        # Save final weights
        final_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "checkpoints", "TITAN_GREEDY_CURRICULUM_FINAL.pt")
