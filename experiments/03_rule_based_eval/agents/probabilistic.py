...
def _get_card(obs: Observation, area: AreaType, index: int, player_index: int) -> Pokemon | Card | None:
    ...
    if area == AreaType.HAND: return player.hand[index] if player.hand else None
    if area == AreaType.DISCARD: return player.discard[index] if index < len(player.discard) else None
    if area == AreaType.ACTIVE: return player.active[index] if index < len(player.active) else None
    if area == AreaType.BENCH: return player.bench[index] if index < len(player.bench) else None
    if area == AreaType.PRIZE: return player.prize[index] if index < len(player.prize) else None
    if area == AreaType.STADIUM: return obs.current.stadium[index] if obs.current.stadium and index < len(obs.current.stadium) else None
...
class AdvancedPolicy:
    ...
    def _scan_main_options(self) -> None:
        ...
        for option in self.select.option:
            if option.type == OptionType.PLAY:
                card = _get_card(self.obs, AreaType.HAND, option.index, self.my_index)
        ...
        for attacker_index, my_pokemon in enumerate(self._my_board()):
            ...
            for attack_index in range(2):
                ...
                for target_index, op_pokemon in enumerate(self._opponent_board()):
                    ...
                    local_prize = prize_count(op_pokemon) if op_pokemon.hp <= damage else 0
                    if local_prize == 0: score *= damage / op_pokemon.hp
                    if len(self.opponent.prize) <= local_prize: score = 500000

                    score += base_score + (220 if attacker_index == 0 else 0) + (300 if target_index == 0 else 0) + energy_count
                    if score > best_score:
                        best_score = score
                        self.plan = (attacker_index, target_index, attack_index, op_pokemon.hp - damage, needs_energy)
    ...
    def _score_option(self, option) -> float:
        if option.type == OptionType.NUMBER: return option.number or 0
        ...
        if option.type == OptionType.RETREAT:
            return 2000 if plan and plan[0] >= 1 else -1
        if option.type == OptionType.ATTACK:
            if plan is None: return 1000
            return 1100 if (option.attackId == MEGA_BRAVE) == (plan[2] == 1) else 1000
        return 0

    def _score_card_choice(self, option) -> float:
        card = _get_card(self.obs, option.area, option.index, option.playerIndex)
        ...
        if self.context == SelectContext.TO_HAND:
            ...
            elif card.id == C.BASIC_FIGHTING_ENERGY: score += 30 if not self.ability_used or not self.state.energyAttached else -1
            return score
    ...
    def _score_active_choice(self, option, card: Pokemon | Card) -> float:
        ...
        if option.playerIndex != self.my_index:
            return 100 if (plan and option.index == plan[1] - 1) else 0
        score = len(card.energies) * 2
        if plan and option.index == plan[0] - 1: score += 100
    ...
    def _score_play(self, option) -> float:
        ...
        if data.cardType == CardType.POKEMON:
            ...
            return 20000
        if card.id == C.SWITCH: return 6000 if plan and plan[0] > 0 else -1
        if card.id == C.PREMIUM_POWER_PRO:
            if self.state.supporterPlayed and plan and plan[3] <= 0: return -1
            if not self.can_attack: return 3050 if (not self.state.supporterPlayed and self.hand_counts[C.CARMINE] > 0 and self.hand_counts[C.LILLIE_DETERMINATION] == 0 and self.me.deckCount > LOW_DECK_COUNT) else -1
            return 5000
        if card.id == C.BOSS_ORDERS: return 3200 if (plan and plan[1] >= 1) else -1
    ...
    def _score_attach(self, option) -> float:
        card = _get_card(self.obs, AreaType.HAND, option.index, self.my_index)
        pokemon = _get_card(self.obs, option.inPlayArea, option.inPlayIndex, self.my_index)
        if not isinstance(pokemon, Pokemon): return 0
        if card and card.id == C.HERO_CAPE:
            score = 7000
        ...
        if plan and board_index == plan[0] and plan[4]: score += 200
        return score
    ...
    def _score_evolve(self, option) -> float:
        ...
        if not isinstance(pokemon, Pokemon): return 0
        plan = getattr(self, 'plan', None)
        if pokemon.id == C.MAKUHITA and plan and plan[1] == 0 and not self._opponent_is_crustle_wall(): return -1
        return 9000 + len(pokemon.energies)

    def _score_ability(self, option) -> float:
        card = _get_card(self.obs, option.area, option.index, self.my_index)
        if card is None: return -1
    ...
    def _remember_lunatone_ability(self, ranked: list[int]) -> None:
        ...
        card = _get_card(self.obs, option.area, option.index, self.my_index)
        if card is not None and card.id == C.LUNATONE: self.ability_used = True
...
class ProbabilisticAgent(BaseAgentClass):
    def __init__(self, deck_path: str):
        # Load deck from the hardcoded DECK list (this agent has its own deck)
        self.deck = list(DECK)
        self.pre_turn = -1
        self.ability_used = False
        self.plan = None
...
_probabilistic_instance = ProbabilisticAgent(deck_path="")

def agent(obs_dict: dict) -> list[int]:
    return _probabilistic_instance(obs_dict)
